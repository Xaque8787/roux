...
from ..models import (InventoryDay, InventoryItem, InventoryDayItem, Task, User, 
...
async def inventory_page(request: Request, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    ...
    # Get employees for day creation
...
async def create_inventory_day(
    ...
    employees_working_str = ",".join(map(str, employees_working))
    ...
    # Create tasks for all daily janitorial tasks
...
async def start_task(
    ...
    # Update task fields to indicate it's started
...
async def pause_task(
    ...
    # Update task fields to indicate it's paused
    task.is_paused = True
...
async def resume_task(
    ...
    # Update task fields to indicate it's resumed
    task.is_paused = False
    
    # Add pause time to total
    if task.paused_at:
        pause_duration = datetime.utcnow() - task.paused_at
        task.total_pause_time += pause_duration.total_seconds()
        task.paused_at = None
...
async def finish_task(
    ...
    # Update task fields to indicate it's finished
...
async def create_inventory_item(
    ...
    item = InventoryItem(
        ...
        par_unit_equals_amount=par_unit_equals_amount if par_unit_equals_amount else None,
        par_unit_equals_unit=par_unit_equals_unit if par_unit_equals_unit else None,
...
async def update_inventory_item(
    ...
    item.par_unit_equals_amount = par_unit_equals_amount if par_unit_equals_amount else None
    item.par_unit_equals_unit = par_unit_equals_unit if par_unit_equals_unit else None
